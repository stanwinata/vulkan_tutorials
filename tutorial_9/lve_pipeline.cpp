...
namespace lve {
    ...
    void LvePipeline::defaultPipelineConfigInfo(PipelineConfigInfo& out_config_info) {
        // Setting up first stage of pipeline/Input Assembler.
        // Takes in list of vertices and group them as geometry.
        out_config_info.inputAssemblyInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;
        ...
        // two vertices.
        out_config_info.inputAssemblyInfo.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
        out_config_info.inputAssemblyInfo.primitiveRestartEnable = VK_FALSE;

        // Fuse scissor and viewport into ViewPortInfo
        // Some GPU can use multiple ViewPortInfo
        out_config_info.viewportInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO;
        out_config_info.viewportInfo.viewportCount = 1;
        // pViewports is set during recording of cmd buffer.
        out_config_info.viewportInfo.pViewports = nullptr;
        out_config_info.viewportInfo.scissorCount = 1;
        // pScissors is set during recording of cmd buffer.
        out_config_info.viewportInfo.pScissors = nullptr;


        // Rasetrization stage: breaks up geometry into fragments for each pixel.
        out_config_info.rasterizationInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO;
        ...
        out_config_info.rasterizationInfo.depthClampEnable = VK_FALSE;
        ...
        out_config_info.rasterizationInfo.rasterizerDiscardEnable = VK_FALSE;
        // For draw triangle: just corners, edges, or line filled in.
        out_config_info.rasterizationInfo.polygonMode = VK_POLYGON_MODE_FILL;
        out_config_info.rasterizationInfo.lineWidth = 1.0f;
        // Can discard triangle based on apparent facing/winding order.
        out_config_info.rasterizationInfo.cullMode = VK_CULL_MODE_NONE;
        // based on the order of vertex + cull order(CW, CCW) we give,
        // we can know which side of triangle we are looking at.
        out_config_info.rasterizationInfo.frontFace = VK_FRONT_FACE_CLOCKWISE;
        // Alter depth value by adding constant value, or multiply by slope factor.
        out_config_info.rasterizationInfo.depthBiasEnable = VK_FALSE;
        out_config_info.rasterizationInfo.depthBiasConstantFactor = 0.0f;  // Optional
        out_config_info.rasterizationInfo.depthBiasClamp = 0.0f;           // Optional
        out_config_info.rasterizationInfo.depthBiasSlopeFactor = 0.0f;     // Optional
        ...
        out_config_info.multisampleInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_MULTISAMPLE_STATE_CREATE_INFO;
        out_config_info.multisampleInfo.sampleShadingEnable = VK_FALSE;
        out_config_info.multisampleInfo.rasterizationSamples = VK_SAMPLE_COUNT_1_BIT;
        out_config_info.multisampleInfo.minSampleShading = 1.0f;           // Optional
        out_config_info.multisampleInfo.pSampleMask = nullptr;             // Optional
        out_config_info.multisampleInfo.alphaToCoverageEnable = VK_FALSE;  // Optional
        out_config_info.multisampleInfo.alphaToOneEnable = VK_FALSE;       // Optional
        ...
        out_config_info.colorBlendAttachment.colorWriteMask =
            VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT |
            VK_COLOR_COMPONENT_A_BIT;
        out_config_info.colorBlendAttachment.blendEnable = VK_FALSE;
        out_config_info.colorBlendAttachment.srcColorBlendFactor = VK_BLEND_FACTOR_ONE;   // Optional
        out_config_info.colorBlendAttachment.dstColorBlendFactor = VK_BLEND_FACTOR_ZERO;  // Optional
        out_config_info.colorBlendAttachment.colorBlendOp = VK_BLEND_OP_ADD;              // Optional
        out_config_info.colorBlendAttachment.srcAlphaBlendFactor = VK_BLEND_FACTOR_ONE;   // Optional
        out_config_info.colorBlendAttachment.dstAlphaBlendFactor = VK_BLEND_FACTOR_ZERO;  // Optional
        out_config_info.colorBlendAttachment.alphaBlendOp = VK_BLEND_OP_ADD;              // Optional

        out_config_info.colorBlendInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO;
        out_config_info.colorBlendInfo.logicOpEnable = VK_FALSE;
        out_config_info.colorBlendInfo.logicOp = VK_LOGIC_OP_COPY;  // Optional
        out_config_info.colorBlendInfo.attachmentCount = 1;
        out_config_info.colorBlendInfo.pAttachments = &out_config_info.colorBlendAttachment;
        out_config_info.colorBlendInfo.blendConstants[0] = 0.0f;  // Optional
        out_config_info.colorBlendInfo.blendConstants[1] = 0.0f;  // Optional
        out_config_info.colorBlendInfo.blendConstants[2] = 0.0f;  // Optional
        out_config_info.colorBlendInfo.blendConstants[3] = 0.0f;  // Optional
        ...
        out_config_info.depthStencilInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_DEPTH_STENCIL_STATE_CREATE_INFO;
        out_config_info.depthStencilInfo.depthTestEnable = VK_TRUE;
        out_config_info.depthStencilInfo.depthWriteEnable = VK_TRUE;
        out_config_info.depthStencilInfo.depthCompareOp = VK_COMPARE_OP_LESS;
        out_config_info.depthStencilInfo.depthBoundsTestEnable = VK_FALSE;
        out_config_info.depthStencilInfo.minDepthBounds = 0.0f;  // Optional
        out_config_info.depthStencilInfo.maxDepthBounds = 1.0f;  // Optional
        out_config_info.depthStencilInfo.stencilTestEnable = VK_FALSE;
        out_config_info.depthStencilInfo.front = {};  // Optional
        out_config_info.depthStencilInfo.back = {};   // Optional

        // Configure pipeline to expect dynamic viewport and scissor to be provided later.
        out_config_info.dynamicStateEnables = {VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR};
        out_config_info.dynamicStateInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO;
        out_config_info.dynamicStateInfo.dynamicStateCount = static_cast<uint32_t>(out_config_info.dynamicStateEnables.size());
        out_config_info.dynamicStateInfo.pDynamicStates = out_config_info.dynamicStateEnables.data();
        out_config_info.dynamicStateInfo.flags = 0;
    }
