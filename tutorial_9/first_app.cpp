...
void FirstApp::RecordCommandBuffer(int image_index) {
  // Setting up frame counter for motion.
  static int frame = 0;
  int repeat_after_num_frame = 300;
  frame = (frame + 1) % repeat_after_num_frame; //modulus to repeat to 0 after 100 frames.
  ...
  for(int j = 0; j < 4; j++) {
    ...
    // Changing the X position by frame*0.01f to make animation.
    push_constant_data.offset = {-0.6f + frame * 0.01f, -0.4f + j * 0.25f};
    push_constant_data.color = {0.0f, 0.0f, 0.2f + j * 0.2f};
    VkShaderStageFlags shader_stages = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
    vkCmdPushConstants(command_buffers_[image_index], pipeline_layout_, shader_stages, /*offset*/ 0, /*size*/sizeof(SimplePushConstantData), &push_constant_data);
